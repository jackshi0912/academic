            ...
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
                ...
                if (cmd == "light") {
                    if (numused == numLights) { // No more Lights 
                        ...
                        if (validinput) {
                            ...
                            for (int i = 4 * numused, j = 0; j < 4; i++, j++) { // Parsing 8 input vlaues
                                lightposn[i] = values[j];
                                lightcolor[i] = values[j + 4];
                            }
                ...
                // Note that no transforms/stacks are applied to the colors. 
                    ...
                    if (validinput) {
                        ...
                        vec3 temp = vec3(values[6], values[7], values[8]);
                        fovy = values[9];

                        vec3 zvec = normalize(eyeinit - center);
                        upinit = Transform::upvector(temp, zvec);
                    }
                }
                ...
                // Also look at demo.txt to get a sense of why things are done this way.
                    ...
                    if (validinput) {
                        ...
                        // Also keep in mind what order your matrix is!
                        float axis_length = sqrt(values[0] * values[0] + values[1] * values[1] + values[2] * values[2]);
                        mat3 rotate = Transform::rotate(values[3], vec3(values[0], values[1], values[2]) / axis_length);
