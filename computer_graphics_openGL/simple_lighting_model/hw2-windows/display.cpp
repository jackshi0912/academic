...
void display() 
{
  ...
  if (numused) {
    ...
    for (int i = 0; i < numused; i++) {
        ...
        GLfloat input[4];
        // Setup input
        input[0] = lightposn[4*i];
        input[1] = lightposn[4*i + 1];
        input[2] = lightposn[4*i + 2];
        input[3] = lightposn[4*i + 3];

        transformvec(input, output);
        lightransf[4*i + 0] = output[0];
        lightransf[4*i + 1] = output[1];
        lightransf[4*i + 2] = output[2];
        lightransf[4*i + 3] = output[3];
    }
  ...
  //transf = Transform::lookAt(eye,center,up)*tr*sc;
